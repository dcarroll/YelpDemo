({
    getYelpData: function(component) {
        ...
        if (action) {
            action.setCallback(this, function(response) {
                console.log(
                    'In the callback in the service for getYelpData...'
                );
    ...
    handleYelpData: function(response, component) {
        ...
        if (state === 'ERROR') {
            component.set(
                'v.errorMessage',
                'We got lost on the way to the data.'
            );
            ...
            toastEvent.setParams({
                ...
                title: 'Ooops!',
                message:
                    'A really bad thing happened on the way to retrieving your data.',
                mode: 'sticky'
            });
            toastEvent.fire();
            ...
            if (data.error) {
                ...
                yelpEvent.setParams({
                    locationUsed: data.location,
                    resultList: data.bizArray
                });
                yelpEvent.fire();
            }
        }
